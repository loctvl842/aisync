...
def combine_sentences(sentences: List[dict], buffer_size: int = 1) -> List[dict]:
    ...
    for i in range(len(sentences)):
        ...
        for j in range(i + 1, min(len(sentences), i + 1 + buffer_size)):
            # Add the sentence at index j to the combined_sentence string
            combined_sentence += " " + sentences[j]["sentence"]
...
def single_distance_calculation(
    ...
    Args:
        embedding_current: Current embedding.
        embedding_next: Next embedding.
        distance_metric: metric used for calculating distance,
        it must be one of ["l2_distance", "l1_distance", "cosine_distance", "max_inner_product"]

    Returns:
        Distance based on distance_metric between two embeddings.
    """
...
def calculate_distances(
    ...
    for i in range(len(sentences) - 1):
        ...
        # Calculate distance
        distance = single_distance_calculation(embedding_current, embedding_next, distance_metric)
...
class SemanticTextSplitter(BaseDocumentTransformer):
    ...
    All credits to him.
    Credit to Langchain for the implementation.
    ...
    def __init__(
        self,
        embeddings: Optional[Embeddings] = OpenAIEmbeddings(),
        buffer_size: int = 1,
    ...
    def _calculate_sentence_distances(self, single_sentences_list: List[str]) -> Tuple[List[float], List[dict]]:
        ...
        return calculate_distances(sentences=sentences, distance_metric=self.distance_metric)
