...
ConditionalBranchAction = tuple["ConditionalBranch", Callable]
...
class Graph:
    ...
    def compile(self):
        ...
        while queue:
            ...
            for edge in current_node.edges:
                if isinstance(edge, Node) and edge.name not in compiled:
    ...
    def run(self, *, max_workers: int = None, max_retries: int = 3):
        ...
        sources = self.get_source()
        if not sources:
            log.warning("No source nodes found. Cannot run the graph")
            return
            ...
            while queue or future_to_node:
                ...
                for future in done:
                    ...
                    try:
                        ...
                        log.info(f"Node {current_node.name} returned {result}")
                    except Exception as e:
                        retries[current_node.name] += 1
                        log.error(f"Error running node {current_node.name}: {e}")
                        continue
                    for edge in current_node.edges:
                        if isinstance(edge, Node):
                            queue.append(edge)
                            ...
                            for name in selected_node_names:
                                ...
                                if branch_node:
                                    ...
                                    raise ValueError(f"Node '{name}' not found in ConditionalBranch '{cond_branch}'.")
                        else:
    ...
    def to_mermaid(self, direction: Literal["TD", "LR", "BT", "RL"] = "TD") -> str:
        ...
        for node in self.nodes.values():
            if isinstance(node, Node):
                for edge in node.edges:
                    if isinstance(edge, tuple) and isinstance(edge[0], ConditionalBranch):
    ...
    def __rshift__(self, other: Union[Node, Graph, Branch, ConditionalBranchAction]) -> Graph:
        if isinstance(other, Node):
            ...
            return Graph(*self.nodes.values(), other)
            ...
            return Graph(*self.nodes.values(), *other.nodes.values())
            ...
            for node in self_sink_nodes:
                node >> other
            return Graph(*self.nodes.values(), *other[0].nodes.values())
        else:
            raise ValueError("Invalid right shift operator")

    def __getattr__(self, node_name: str):
        if node_name in self.nodes:
            return SubGraph(self.nodes[node_name], self)
...
class Branch:
    ...
    def __and__(self, other: Union[Node, Branch]) -> Branch:
        if isinstance(other, Node):
            # Branch & Node
            return Branch(*self.nodes.values(), other)
        elif isinstance(other, Branch):
            # Branch & Branch
            return Branch(*self.nodes.values(), *other.nodes.values())
        else:
...
class ConditionalBranch:
    ...
    def __or__(self, other: Union[Node, ConditionalBranch]) -> ConditionalBranch:
        if isinstance(other, Node):
            ...
            return ConditionalBranch(*self.nodes.values(), other)
        elif isinstance(other, ConditionalBranch):
            # ConditionalBranch | ConditionalBranch
            return ConditionalBranch(*self.nodes.values(), *other.nodes.values())
        else:
...
class Node:
    ...
    def __rshift__(self, other: Union[Node, Graph, Branch, ConditionalBranchAction]) -> Graph:
        if isinstance(other, Node):
            ...
            return Graph(self, *other.nodes.values())
        elif isinstance(other, Branch):
            # Node >> Branch
            ...
            self.edges.append(other)
            return Graph(self, *branch.nodes.values())
        else:
            raise ValueError("Invalid right shift operator")
    ...
    def __and__(self, other: Union[Node, Branch]) -> Branch:
        if isinstance(other, Node):
            ...
            # Node & Branch
            return Branch(self, *other.nodes.values())
        else:
            raise ValueError("Invalid and operator")
    ...
    def __or__(self, other: Union[Node, ConditionalBranch]) -> ConditionalBranch:
        if isinstance(other, Node):
            # Node | Node
            return ConditionalBranch(self, other)
        elif isinstance(other, ConditionalBranch):
